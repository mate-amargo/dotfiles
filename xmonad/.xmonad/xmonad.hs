...
import XMonad.Operations -- unGrab for scrot -s
...
promptTheme = def
    {
      font                  = "xft:ComicShannsMono Nerd Font-12"
    , bgColor               = "#646464"
    , fgColor               = "#FF4000"
    , promptBorderWidth     = 0
    , position              = Top
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = (subtitle "Custom Keys":) $
  [
    ...
    , ((modm, xK_r     ), addName "Opens dmenu launcher" $ spawn "dmenu_run -fn 'ComicShannsMono Nerd Font-12'")
    ...
    -- Brightness
    , ((0, xF86XK_MonBrightnessUp), addName "Increase Brightness" $ spawn "backlight +500")
    , ((0, xF86XK_MonBrightnessDown), addName "Decrease Brightness" $ spawn "backlight -500")
    -- Apps
    , ((modm, xK_i ), addName "Web Browser" $ spawn "google-chrome-stable")
    , ((modm, xK_m), addName "Music Player" $ spawnOn wsMSX "spotify")
    ...
    -- Passwords
    , ((modm, xK_p ), addName "Password Manager autotype" $ spawn "passmenu --type -fn 'ComicShannsMono Nerd Font-12'")
    , ((modm .|. controlMask, xK_p ), addName "Password Manager clipboard" $ spawn "passmenu -fn 'ComicShannsMono Nerd Font-12'")
    -- Scratchpads
    ...
    , ((modm .|. shiftMask, xK_Tab   ), addName "Toggle between previous screen" $ prevScreen)
    ...
    ++
    [((m .|. modm, key), addName (s ++ show sc ++ ".") $ screenWorkspace sc >>= flip whenJust (windows . f))
        | (key, sc) <- zip [xK_a, xK_s] [0..]
        , (f, m, s) <- [(W.view, 0, "Switch to physical/Xinerama screen "), (W.shift, shiftMask, "Move client to screen ")]]
--------------------------------------------}}}
-- Mouse bindings                           {{{
-----------------------------------------------
...
wsCHT = "\xf086" -- 9: Chat 
myWorkspaces = [wsWEB, wsCMD, wsMSX, wsBOK, wsGAM, wsPIC, wsKNW, wsMAT, wsCHT]
...
tabbedConf = def {
    fontName = "xft:ComicShannsMono Nerd Font-12"
...
scratchpads = [ NS "htop" (myTerminal ++ " start --class scratchpad-htop -- htop") (className =? "scratchpad-htop") floatingWindow,
                NS "XMonadConfig" (myTerminal ++ " start --class scratchpad-xmonadconfig -- vim ~/.xmonad/xmonad.hs") (className =? "scratchpad-xmonadconfig") floatingWindow,
                NS "terminal" (myTerminal ++ " start --class scratchpad-terminal") (className =? "scratchpad-terminal") floatingWindow
...
myStartupHook = setDefaultCursor xC_left_ptr <+> setWMName "LG3D" <+> spawnOnce "stalonetray" <+> myExclusives
...
myPP = xmobarPP { ppCurrent = xmobarColor "#FF4000" ""
                ...
                , ppSort = fmap (filterOutWs ["NSP"] .) (ppSort def)
                }
